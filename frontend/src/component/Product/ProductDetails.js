...
const ProductDetails = () => {
  ...
  const handelCountIncrease = () => {
    ...
    } else toast.error("No more items left");
  };
  ...
  return (
    <>
      ...
      <div className=" max-w-5xl mx-auto p-4 flex items-center lg:flex-row lg:items-start flex-col lg:items-top gap-10 m-5 justify-center ">
        ...
        <div className=" p-5 scrollbar border shadow-lg xs:w-3/4 md:w-1/2">
          ...
          <p
            ...
            className="text-sm font-thin text-slate-600 border-b border-slate-400 mb-4 pb-4"
          >
            #{product._id}
          </p>
          <ReactStars {...options} />
          <p className="border-b border-slate-400 mb-4 pb-4">
            ({product.numberOfReviews} Reviews)
          </p>
          <h2 className="text-black-500 text-3xl font-bold">
            ${product.price} <br />
            <p className="text-sm font-thin text-slate-600 border-b border-slate-400 mb-4 pb-4">
              {" "}
          ...
          <div className="flex items-center ">
            ...
            <button
              className="w-40 h-[40px] my-4 mx-4 bg-blue-500 rounded-md active:bg-blue-700 text-white font-bold duration-500"
          ...
          <p className="border-b border-slate-400 mb-4 pb-4 font-bold">
            Status:{" "}
            <span
              className={`font-normal ${
                product.stock < 1 ? `text-red-400` : "text-green-400"
              }`}
            >{`${
              product.stock < 1
                ? `Out of Stock`
                : `Only ${product.stock} Unit left`
            }`}</span>{" "}
          </p>
          <p>
      ...
      <div className=" w-3/4  max-w-3xl flex p-4 lg:px-12  flex-col gap-2 border-y border-dashed mt-32 mb-10 mx-auto items-center justify-between sm:flex-row ">
        ...
        <button
          className="text-white  font-medium w-[200px] h-[40px] bg-blue-200 rounded-lg"
        ...
        <div
          className={`inset-0 z-10 fixed flex justify-center items-center bg-black backdrop-filter bg-opacity-50 backdrop-blur-md ${
            toggle ? `block` : `hidden`
          }`}
        >
          <AddReview
