...
const LoginUserCart = () => {
  ...
  const { data, status, isProductRemovedFromCart } = useSelector(
    (state) => state.cart
  );
  ...
  const handelCheckout = () => {
    toast.success("Rent request has been placed!");
    // const storedShippingData = localStorage.getItem("shippingData");
    // if (!storedShippingData) navigate("/cart/checkout");
    // else navigate("/cart/shippingInfo");
  };
        ...
        <div className="md:max-w-[70%] max-w-[90%] p-2 mt-14 w-full rounded-md bg-slate-700 bg-opacity-20">
          ...
          {products.length > 0 ? (
            <div className="text-right p-5">
              ...
              <button onClick={handelCheckout} className="bg-orange-400 p-2">
                Rent
              </button>
            </div>
          ) : (
            <div className="text-center h-44 font-bold">
              ...
              <Link to="/">
                <button className="bg-blue-500 p-2 m-2 rounded-sm">
                  Continue Shopping
                </button>
              </Link>
            </div>
