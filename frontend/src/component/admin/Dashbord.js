...
import {
  getAdminProducts,
  getAllUsers,
} from "../../slices/adminSlice/adminSlice";
...
const Dashbord = () => {
  ...
  if (status === STATUSES.LOADING) {
    return (
      <div class="w-full grid place-content-center h-screen ">
        <Loader />
      </div>
    );
  }
  return (
    <>
      ...
      <div className="bg-slate-800 w-screen min-h-screen absolute top-0 z-10 py-20">
        <FaHome
          onClick={() => navigate("/")}
          className="text-white cursor-pointer absolute left-5 top-5 text-3xl "
        />
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mt-1 mb-10 mx-5 text-white rounded-sm  gap-1 ">
            <Link
              ...
              className="bg-orange-500 p-2 rounded-sm text-center text-white font-bold"
            >
              Manage Product
            </Link>
            <Link
              to="/admin/manageuser"
              className="bg-green-600 p-2 rounded-sm text-center text-white font-bold"
            >
              Manage Users
            </Link>
